...
void __global__ rakMoveIterationThreadCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, K *bufn, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS)
{
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomt = 0;
  for (K u = NB + B * b + t; u < NE; u += G * B)
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakMoveIterationThreadCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, K *bufn, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS)
{
  const int B = blockSizeCu(NE - NB, BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu(NE - NB, B, GRID_LIMIT_MAP_CUDA);
  rakMoveIterationThreadCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufn, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 */
template <class O, class K, class V, class W, class F>
void __global__ rakMoveIterationBlockCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, K *bufn, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS)
{
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomb = 0;
  ...
  if (t == 0)
    atomicAdd((uint64_cu *)ncom, ncomb);
}
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakMoveIterationBlockCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, K *bufn, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS)
{
  const int B = blockSizeCu<true>(NE - NB, BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu<true>(NE - NB, B, GRID_LIMIT_MAP_CUDA);
  rakMoveIterationBlockCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufn, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
#pragma endregion
...
void __global__ rakCrossCheckCukU(uint64_cu *ncom, K *vcom, K *vdom, K NB, K NE)
{
  ...
  for (K u = NB + B * b + t; u < NE; u += G * B)
  {
    ...
    if (vdom[c] == c)
      ...
      atomicAdd(ncom, uint64_cu() - 1); // Bad things happened
  }
}
...
inline int rakLoopCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, K *bufn, W *bufw, const O *xoff, const K *xedg, const V *xwei, K N, K NL, double E, int L)
{
  ...
  while (l < L)
  {
    ...
    rakMoveIterationThreadCuU(ncom, vcom, vaff, bufk, bufn, bufw, xoff, xedg, xwei, K(), N, PICKLESS);
    // rakMoveIterationBlockCuU (ncom, vcom, vaff, bufk, bufn, bufw, xoff, xedg, xwei, NL,  N,  PICKLESS);
    ++l;
    // rakCrossCheckCuU(ncom, vdom, vcom, K(), N); swap(vdom, vcom);
    TRY_CUDA(cudaMemcpy(&n, ncom, sizeof(uint64_cu), cudaMemcpyDeviceToHost));
    if (!PICKLESS && double(n) / N <= E)
      break;
  }
  return l;
}
...
{
  auto fm = [&](auto &vaff)
  { rakAffectedVerticesFrontierOmpW(vaff, y, deletions, insertions, *q); };
