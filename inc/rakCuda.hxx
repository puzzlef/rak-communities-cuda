 ...
 */
template <bool SELF=false, bool BLOCK=false, int HTYPE=0, class O, class K, class V, class W>
 ...
 */
template <int BLIM=32, int HTYPE=0, class O, class K, class V, class W, class F>
...
inline void rakMoveIterationThreadCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  ...
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakMoveIterationThreadCukU<BLIM, HTYPE><<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 * @param PICKLESS allow only picking smaller community id?
 */
template <int BLIM=128, int HTYPE=0, class O, class K, class V, class W, class F>
void __global__ rakMoveIterationBlockCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  ...
  for (K u=NB+b; u<NE; u+=G) {
    ...
    size_t H  = nextPow2Cud(EN) - 1;
    size_t T  = nextPow2Cud(H)  - 1;
    K *hk = bufk + 2*EO;  // EN <= MAX_DEGREE? shrk : bufk + 2*EO;
    W *hv = bufw + 2*EO;  // EN <= MAX_DEGREE? shrw : bufw + 2*EO;
...
inline void rakMoveIterationBlockCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  ...
  rakMoveIterationBlockCukU<BLIM, HTYPE><<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 */
template <int HTYPE=0, class O, class K, class V, class W, class F>
inline int rakLoopCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K N, K NL, double E, int L) {
  ...
  while (l<L) {
    ...
    rakMoveIterationThreadCuU<32, HTYPE>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, K(), NL, PICKLESS);
    rakMoveIterationBlockCuU<128, HTYPE>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NL,  N,  PICKLESS); ++l;
 ...
 * @param fo process output data (vcom, vcomD)
 * @returns rak result
 */
template <int HTYPE=3, class G, class FI, class FM, class FO>
inline auto rakInvokeCuda(const G& x, const RakOptions& o, FI fi, FM fm, FO fo) {
  ...
  // Obtain CSR representation of the graph.
  ...
  float t  = measureDuration([&]() {
    // Initialize community membership [device].
    ti += measureDuration([&]() { fi(vcomD); });
    // Mark affected vertices [device].
    tm += measureDuration([&]() { fm(vaffD); });
    // Perform iterations.
    l = rakLoopCuU<HTYPE>(ncomD, vcomD, vaffD, bufkD, bufwD, xoffD, xedgD, xweiD, K(N), K(NL), E, L);
  }, o.repeat);
  // Obtain community membership.
  vector<K> vcom(S);
  fo(vcom, vcomD);
  // Free device memory.
...
inline auto rakStaticCuda(const G& x, const RakOptions& o={}) {
  ...
  size_t N = x.order();
  vector<K> ks = vertexKeys(x);
  auto  fi = [&](K *vcomD) { rakInitializeCuW(vcomD, K(), K(N)); };
  auto  fm = [&](F *vaffD) { fillValueCuW(vaffD, N, F(1)); };
  auto  fo = [&](auto& vcom, const K *vcomD) {
    vector<K> vcomc(N);
    TRY_CUDA( cudaMemcpy(vcomc.data(), vcomD, N * sizeof(K), cudaMemcpyDeviceToHost) );
    gatherValuesOmpW(vcom, vcomc, ks);
  };
  return rakInvokeCuda<HTYPE>(x, o, fi, fm, fo);
}
#pragma endregion
#pragma endregion
