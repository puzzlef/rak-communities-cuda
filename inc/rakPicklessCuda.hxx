 ...
 */
template <class K>
void __global__ rakPicklessInitializeCukW(K *vcom, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
...
inline void __device__ rakPicklessScanCommunitiesCudU(K *hk, W *hv, size_t H, size_t T, const O *xoff, const K *xedg, const V *xwei, K u, const K *vcom, size_t i, size_t DI) {
  size_t EO = xoff[u];
...
inline void __device__ rakPicklessMarkNeighborsCudU(F *vaff, const O *xoff, const K *xedg, K u, size_t i, size_t DI) {
  size_t EO = xoff[u];
  size_t EN = xoff[u+1] - xoff[u];
  for (; i<EN; i+=DI) {
    K v = xedg[EO+i];
...
void __global__ rakPicklessMoveIterationThreadCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS=false) {
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomt = 0;
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    if (!vaff[u]) continue;
    ...
    rakPicklessScanCommunitiesCudU(hk, hv, H, T, xoff, xedg, xwei, u, vcom, 0, 1);
    ...
    vcom[u] = c; ++ncomt;
    rakPicklessMarkNeighborsCudU(vaff, xoff, xedg, u, 0, 1);
  }
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakPicklessMoveIterationThreadCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS=false) {
  const int B = blockSizeCu(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakPicklessMoveIterationThreadCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 */
template <class O, class K, class V, class W, class F>
void __global__ rakPicklessMoveIterationBlockCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS=false) {
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomb = 0;
  for (K u=NB+b; u<NE; u+=G) {
    ...
    __syncthreads();
    rakPicklessScanCommunitiesCudU<false, true>(hk, hv, H, T, xoff, xedg, xwei, u, vcom, t, B);
    __syncthreads();
    // Find best community for u.
    ...
    if (t==0) vcom[u] = c;
    if (t==0) ++ncomb;
    rakPicklessMarkNeighborsCudU(vaff, xoff, xedg, u, t, B);
  }
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakPicklessMoveIterationBlockCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS=false) {
  const int B = blockSizeCu<true>(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu <true>(NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakPicklessMoveIterationBlockCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
#pragma endregion
...
inline size_t rakPicklessPartitionVerticesCudaU(vector<K>& ks, const G& x) {
...
inline RakResult<K> rakPicklessInvokeCuda(const G& x, const vector<K>* q, const RakOptions& o, FM fm) {
  ...
  size_t NL = rakPicklessPartitionVerticesCudaU(ks, x);
  // Obtain data for CSR.
  ...
  float t  = measureDurationMarked([&](auto mark) {
    ...
    else   rakPicklessInitializeCuW(vcomD, K(), K(N));
