 ...
 */
template <class K>
void __global__ rakSharedInitializeCukW(K *vcom, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
...
inline void __device__ rakSharedScanCommunitiesCudU(K *hk, W *hv, size_t H, size_t T, const O *xoff, const K *xedg, const V *xwei, K u, const K *vcom, size_t i, size_t DI) {
  size_t EO = xoff[u];
...
inline void __device__ rakSharedMarkNeighborsCudU(F *vaff, const O *xoff, const K *xedg, K u, size_t i, size_t DI) {
  size_t EO = xoff[u];
  size_t EN = xoff[u+1] - xoff[u];
  for (; i<EN; i+=DI) {
    K v = xedg[EO+i];
...
void __global__ rakSharedMoveIterationThreadCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  DEFINE_CUDA(t, b, B, G);
  ...
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    ...
    rakSharedScanCommunitiesCudU(hk, hv, H, T, xoff, xedg, xwei, u, vcom, 0, 1);
    ...
    vcom[u] = c; ++ncomb[t];
    rakSharedMarkNeighborsCudU(vaff, xoff, xedg, u, 0, 1);
  }
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakSharedMoveIterationThreadCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  const int B = blockSizeCu(NE-NB,   BLOCK_LIMIT_RAK_THREAD_CUDA);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakSharedMoveIterationThreadCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 */
template <class O, class K, class V, class W, class F>
void __global__ rakSharedMoveIterationBlockCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomb = 0;
  for (K u=NB+b; u<NE; u+=G) {
    ...
    __syncthreads();
    rakSharedScanCommunitiesCudU<false, true>(hk, hv, H, T, xoff, xedg, xwei, u, vcom, t, B);
    __syncthreads();
    // Find best community for u.
    ...
    if (t==0) vcom[u] = c;
    if (t==0) ++ncomb;
    rakSharedMarkNeighborsCudU(vaff, xoff, xedg, u, t, B);
  }
  // Update number of changed vertices.
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakSharedMoveIterationBlockCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  const int B = blockSizeCu<true>(NE-NB,   BLOCK_LIMIT_RAK_BLOCK_CUDA);
  const int G = gridSizeCu <true>(NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakSharedMoveIterationBlockCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NB, NE, PICKLESS);
}
#pragma endregion
...
template <class K>
void __global__ rakSharedCrossCheckCukU(uint64_cu *ncom, K *vcom, K *vdom, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    K c = vdom[u];
...
template <class K>
inline void rakSharedCrossCheckCuU(uint64_cu *ncom, K *vcom, K *vdom, K NB, K NE) {
  const int B = blockSizeCu(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakSharedCrossCheckCukU<<<G, B>>>(ncom, vcom, vdom, NB, NE);
}
#pragma endregion
 ...
 */
template <class O, class K, class V, class W, class F>
inline int rakSharedLoopCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, const O *xoff, const K *xedg, const V *xwei, K N, K NL, double E, int L) {
  int l = 0;
  uint64_cu n = 0;
  ...
  while (l<L) {
    ...
    rakSharedMoveIterationThreadCuU(ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, K(), NL, PICKLESS);
    rakSharedMoveIterationBlockCuU (ncom, vcom, vaff, bufk, bufw, xoff, xedg, xwei, NL,  N,  PICKLESS); ++l;
    // rakSharedCrossCheckCuU(ncom, vdom, vcom, K(), N); swap(vdom, vcom);
...
inline size_t rakSharedPartitionVerticesCudaU(vector<K>& ks, const G& x) {
...
inline RakResult<K> rakSharedInvokeCuda(const G& x, const vector<K>* q, const RakOptions& o, FM fm) {
  ...
  size_t NL = rakSharedPartitionVerticesCudaU(ks, x);
  // Obtain data for CSR.
  ...
  float t  = measureDurationMarked([&](auto mark) {
    ...
    else   rakSharedInitializeCuW(vcomD, K(), K(N));
    ...
    // Perform RAK iterations.
    mark([&]() { l = rakSharedLoopCuU(ncomD, vcomD, vaffD, bufkD, bufwD, xoffD, xedgD, xweiD, K(N), K(NL), E, L); });
  }, o.repeat);
...
inline RakResult<K> rakSharedStaticCuda(const G& x, const vector<K>* q=nullptr, const RakOptions& o={}) {
  auto fm = [](auto& vaff) { fillValueOmpU(vaff, FLAG(1)); };
  return rakSharedInvokeCuda<FLAG>(x, q, o, fm);
}
#pragma endregion
...
inline RakResult<K> rakSharedDynamicFrontierCuda(const G& y, const vector<tuple<K, K>>& deletions, const vector<tuple<K, K, V>>& insertions, const vector<K>* q, const RakOptions& o={}) {
  auto fm = [&](auto& vaff) { rakAffectedVerticesFrontierOmpW(vaff, y, deletions, insertions, *q); };
  return rakSharedInvokeCuda<FLAG>(y, q, o, fm);
}
#pragma endregion
#pragma endregion
