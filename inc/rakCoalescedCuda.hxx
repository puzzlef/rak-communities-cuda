...
#include "_main.hxx"
#include "rak.hxx"
#include "hashtableCoalescedCuda.hxx"
 ...
 */
template <class K>
void __global__ rakCoalescedInitializeCukW(K *vcom, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
...
inline void __device__ rakCoalescedScanCommunitiesCudU(K *hk, W *hv, K *hn, size_t H, size_t T, const O *xoff, const K *xedg, const V *xwei, K u, const K *vcom, size_t i, size_t DI) {
  size_t EO = xoff[u];
  ...
  for (; i<EN; i+=DI) {
    ...
    hashtableCoalescedAccumulateCudU<BLOCK>(hk, hv, hn, H, T, c+1, w);
  }
}
...
inline void __device__ rakCoalescedMarkNeighborsCudU(F *vaff, const O *xoff, const K *xedg, K u, size_t i, size_t DI) {
  size_t EO = xoff[u];
  size_t EN = xoff[u+1] - xoff[u];
  for (; i<EN; i+=DI) {
    K v = xedg[EO+i];
...
void __global__ rakCoalescedMoveIterationThreadCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, K *bufn, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  DEFINE_CUDA(t, b, B, G);
  ...
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    ...
    W *hv = bufw + 2*EO;
    K *hn = bufn + 2*EO;
    hashtableCoalescedClearCudW(hk, hv, hn, H, 0, 1);
    rakCoalescedScanCommunitiesCudU(hk, hv, hn, H, T, xoff, xedg, xwei, u, vcom, 0, 1);
    // Find best community for u.
    hashtableCoalescedMaxCudU(hk, hv, H, 0, 1);
    ...
    vcom[u] = c; ++ncomb[t];
    rakCoalescedMarkNeighborsCudU(vaff, xoff, xedg, u, 0, 1);
  }
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakCoalescedMoveIterationThreadCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, K *bufn, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  const int B = blockSizeCu(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakCoalescedMoveIterationThreadCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, bufn, xoff, xedg, xwei, NB, NE, PICKLESS);
}
 ...
 */
template <class O, class K, class V, class W, class F>
void __global__ rakCoalescedMoveIterationBlockCukU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, K *bufn, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  DEFINE_CUDA(t, b, B, G);
  uint64_cu ncomb = 0;
  for (K u=NB+b; u<NE; u+=G) {
    ...
    W *hv = bufw + 2*EO;
    K *hn = bufn + 2*EO;
    hashtableCoalescedClearCudW(hk, hv, hn, H, t, B);
    __syncthreads();
    rakCoalescedScanCommunitiesCudU<false, true>(hk, hv, hn, H, T, xoff, xedg, xwei, u, vcom, t, B);
    __syncthreads();
    // Find best community for u.
    hashtableCoalescedMaxCudU<true>(hk, hv, H, t, B);
    __syncthreads();
    ...
    if (t==0) vcom[u] = c;
    if (t==0) ++ncomb;
    rakCoalescedMarkNeighborsCudU(vaff, xoff, xedg, u, t, B);
  }
  // Update number of changed vertices.
 ...
 */
template <class O, class K, class V, class W, class F>
inline void rakCoalescedMoveIterationBlockCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, K *bufn, const O *xoff, const K *xedg, const V *xwei, K NB, K NE, bool PICKLESS) {
  const int B = blockSizeCu<true>(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu <true>(NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakCoalescedMoveIterationBlockCukU<<<G, B>>>(ncom, vcom, vaff, bufk, bufw, bufn, xoff, xedg, xwei, NB, NE, PICKLESS);
}
#pragma endregion
...
template <class K>
void __global__ rakCoalescedCrossCheckCukU(uint64_cu *ncom, K *vcom, K *vdom, K NB, K NE) {
  DEFINE_CUDA(t, b, B, G);
  for (K u=NB+B*b+t; u<NE; u+=G*B) {
    K c = vdom[u];
...
template <class K>
inline void rakCoalescedCrossCheckCuU(uint64_cu *ncom, K *vcom, K *vdom, K NB, K NE) {
  const int B = blockSizeCu(NE-NB,   BLOCK_LIMIT_MAP_CUDA);
  const int G = gridSizeCu (NE-NB, B, GRID_LIMIT_MAP_CUDA);
  rakCoalescedCrossCheckCukU<<<G, B>>>(ncom, vcom, vdom, NB, NE);
}
#pragma endregion
...
inline int rakCoalescedLoopCuU(uint64_cu *ncom, K *vcom, F *vaff, K *bufk, W *bufw, K *bufn, const O *xoff, const K *xedg, const V *xwei, K N, K NL, double E, int L) {
  int l = 0;
  uint64_cu n = 0;
  while (l<L) {
    bool PICKLESS = l % PICKSTEP == 0;
    fillValueCuW(ncom, 1, uint64_cu());
    rakCoalescedMoveIterationThreadCuU(ncom, vcom, vaff, bufk, bufw, bufn, xoff, xedg, xwei, K(), NL, PICKLESS);
    rakCoalescedMoveIterationBlockCuU (ncom, vcom, vaff, bufk, bufw, bufn, xoff, xedg, xwei, NL,  N,  PICKLESS); ++l;
    // rakCoalescedCrossCheckCuU(ncom, vdom, vcom, K(), N); swap(vdom, vcom);
...
inline size_t rakCoalescedPartitionVerticesCudaU(vector<K>& ks, const G& x) {
...
inline RakResult<K> rakCoalescedInvokeCuda(const G& x, const vector<K>* q, const RakOptions& o, FM fm) {
  ...
  size_t NL = rakCoalescedPartitionVerticesCudaU(ks, x);
  // Obtain data for CSR.
  ...
  TRY_CUDA( cudaMalloc(&bufnD, (2*M) * sizeof(K)) );
  ...
  float t  = measureDurationMarked([&](auto mark) {
    ...
    else   rakCoalescedInitializeCuW(vcomD, K(), K(N));
    ...
    // Perform RAK iterations.
    mark([&]() { l = rakCoalescedLoopCuU(ncomD, vcomD, vaffD, bufkD, bufwD, bufnD, xoffD, xedgD, xweiD, K(N), K(NL), E, L); });
  }, o.repeat);
...
inline RakResult<K> rakCoalescedStaticCuda(const G& x, const vector<K>* q=nullptr, const RakOptions& o={}) {
  auto fm = [](auto& vaff) { fillValueOmpU(vaff, FLAG(1)); };
  return rakCoalescedInvokeCuda<FLAG>(x, q, o, fm);
}
#pragma endregion
...
inline RakResult<K> rakCoalescedDynamicFrontierCuda(const G& y, const vector<tuple<K, K>>& deletions, const vector<tuple<K, K, V>>& insertions, const vector<K>* q, const RakOptions& o={}) {
  auto fm = [&](auto& vaff) { rakAffectedVerticesFrontierOmpW(vaff, y, deletions, insertions, *q); };
  return rakCoalescedInvokeCuda<FLAG>(y, q, o, fm);
}
#pragma endregion
#pragma endregion
